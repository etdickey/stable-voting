...
def run_pref_voting(tournaments: list[Tournament], repeats: int,
                    sv_algorithm: str, ssv_algorithm: str) -> tuple[
                        list[TimingResult], list[TimingResult], list[float], PrefRunTiming
                    ]:
    ssv_results = []
    sv_results = []
    graph_times = []
    ...
    for index, tournament in enumerate(tournaments, start=1):
        ...
        print(
            f"benchmarked {index}/{len(tournaments)}: N={tournament.candidates}, "
    ...
    graph_seconds = sum(graph_times)
    ssv_seconds = sum(result.wall_seconds for result in ssv_results)
    sv_seconds = sum(result.wall_seconds for result in sv_results)
...
def combine_results(
    ...
    pref_ssv: list[TimingResult],
    cpp_sv: list[TimingResult],
    pref_sv: list[TimingResult],
    graph_times: list[float],
) -> list[dict]:
    rows = []

    for i, tournament in enumerate(tournaments):
        if cpp_ssv[i].winner != pref_ssv[i].winner:
            raise RuntimeError(
        ...
        if cpp_sv[i].winner != pref_sv[i].winner:
            raise RuntimeError(
        ...
        rows.append({
            ...
            "pref_ssv_seconds": pref_ssv[i].median_seconds,
            "cpp_sv_seconds": cpp_sv[i].median_seconds,
            "pref_sv_seconds": pref_sv[i].median_seconds,
            ...
            "pref_ssv_warmup_seconds": pref_ssv[i].warmup_seconds,
            "pref_ssv_gc_seconds": pref_ssv[i].setup_seconds,
            "pref_ssv_timed_total_seconds": pref_ssv[i].timed_total_seconds,
            "pref_ssv_benchmark_wall_seconds": pref_ssv[i].wall_seconds,
            "pref_ssv_harness_overhead_seconds": pref_ssv[i].overhead_seconds,
            ...
            "pref_sv_warmup_seconds": pref_sv[i].warmup_seconds,
            "pref_sv_gc_seconds": pref_sv[i].setup_seconds,
            "pref_sv_timed_total_seconds": pref_sv[i].timed_total_seconds,
            "pref_sv_benchmark_wall_seconds": pref_sv[i].wall_seconds,
            "pref_sv_harness_overhead_seconds": pref_sv[i].overhead_seconds,
        })
...
def summarize(raw_rows: list[dict]) -> list[dict]:
    ...
    for n in sorted({row["candidates"] for row in raw_rows}):
        ...
        pref_ssv = [r["pref_ssv_seconds"] for r in group]
        cpp_sv = [r["cpp_sv_seconds"] for r in group]
        pref_sv = [r["pref_sv_seconds"] for r in group]

        row = {
            "candidates": n,
            "tournaments": len(group),
            "cpp_ssv_ms": 1000 * statistics.median(cpp_ssv),
            "pref_ssv_ms": 1000 * statistics.median(pref_ssv),
            "cpp_sv_ms": 1000 * statistics.median(cpp_sv),
            "pref_sv_ms": 1000 * statistics.median(pref_sv),
            "cpp_ssv_mean_ms": 1000 * statistics.mean(cpp_ssv),
            "pref_ssv_mean_ms": 1000 * statistics.mean(pref_ssv),
            "cpp_sv_mean_ms": 1000 * statistics.mean(cpp_sv),
            "pref_sv_mean_ms": 1000 * statistics.mean(pref_sv),
            "cpp_ssv_max_ms": 1000 * max(cpp_ssv),
            "pref_ssv_max_ms": 1000 * max(pref_ssv),
            "cpp_sv_max_ms": 1000 * max(cpp_sv),
            "pref_sv_max_ms": 1000 * max(pref_sv),
        }
        row["ssv_speedup"] = row["pref_ssv_ms"] / row["cpp_ssv_ms"]
        row["sv_speedup"] = row["pref_sv_ms"] / row["cpp_sv_ms"]
        row["ssv_mean_speedup"] = row["pref_ssv_mean_ms"] / row["cpp_ssv_mean_ms"]
        row["sv_mean_speedup"] = row["pref_sv_mean_ms"] / row["cpp_sv_mean_ms"]
        summary.append(row)
    return summary
...
def print_table(summary: list[dict]) -> None:
    ...
    last = summary[-1]
    print(
        f"\nAt N={last['candidates']}, the largest per-tournament medians were "
        f"{last['pref_ssv_max_ms'] / 1000:.6g}s for pref_voting SSV and "
        f"{last['pref_sv_max_ms'] / 1000:.6g}s for pref_voting SV."
    )
